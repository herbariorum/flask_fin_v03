...
class FormLogin(FlaskForm):
    email = wtforms.EmailField('Email', validators=[DataRequired(message="O campo é requerido"), Email(message="Endereço de email inválido")])
    password_hash = wtforms.PasswordField('Password', validators=[DataRequired(message="O campo é requerido"), Length(min=6, message="O campo deve ter no mínimo 6 dígitos")])
    submit = wtforms.SubmitField('Submit')